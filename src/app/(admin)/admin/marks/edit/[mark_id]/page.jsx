  ...
  useEffect(() => {
    ...
    ApiService.get(`${GET_MARK_BY_ID_ROUTE}/${markId}`)
      .then((resp) => {
        ...
        const translations = Array.isArray(mark?.translations) ? mark.translations : [];
        ...
        if (!next[`name_${CONTENT_LANGUAGES.AZ}`]) next[`name_${CONTENT_LANGUAGES.AZ}`] = mark?.name ?? "";
        if (!next.image) next.image = mark?.image ?? "";
            ...
            <div className="col-lg-12">
              ...
              <SgFormGroup>
                <SgInput
                  ...
                  label={`Marka adı (${activeLang.toUpperCase()})`}
