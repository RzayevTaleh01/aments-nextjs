...
export default function Index() {
    ...
    const { query: {announcement_id} } = router;
    ...
    function handleSubmit(e) {
        ...
        let errors = validate(data, 'announcementCreate', validationConstraints);

        if (Object.keys(errors).length > 0) {
            setValueErrors(errors)
        }
        else {
            ApiService.put(`${EDIT_ANNOUNCEMENT_BY_ID_ROUTE}/${announcement_id}`, {data}).then(async () => {
                await router.push({
                    pathname: '/content/idareedici/announcements/'
    ...
    useEffect(() => {
        ApiService.get(`${GET_ANNOUNCEMENT_BY_ID_ROUTE}/${announcement_id}`).then(resp => {
            ...
            <SgPage>
                ...
                >
                    <SgButton
                        type='link'
                        isLinked={true}
                        to='/content/idareedici/announcements'
                        color='primary'
                        size='md'
                    >
                        Elanlar
