...
export default function Index() {
    ...
    const {query: {school_id, region_id, grade_id, subject_id, chapter_id, topic_id, key}} = router;
    ...
    async function getInfos() {
        ...
        if (topic_id) {
            await ApiService.get(`${GET_INFO_BY_CATEGORY_ID_ROUTE}/${topic_id}`).then(resp => {
                object = ({...object, topic: resp.data.data})
            }).catch(err => {
                console.log(err);
            })
        }
    ...
    useEffect(() => {
        getInfos()
    }, [school_id, region_id, grade_id, subject_id, chapter_id, topic_id])
    ...
    return (
        <>
            <SgPage>
                <SgPageHead
                    header='Şagird listi'
                    description={`${infoData?.region?.name ? `<b>${infoData?.region?.name}</b> rayon, ` : ''}${infoData?.school?.name ? `<b>${infoData?.school?.name}</b>, ` : ''}${infoData?.grade?.name ? `<b>${infoData?.grade?.name}</b> sinif,` : ''}${infoData?.subject?.name ? `<b>${infoData?.subject?.name}</b> fənni, ` : ''}${infoData?.chapter?.name ? `<b>${infoData?.chapter?.name}</b> bölməsi ` : ''}${infoData?.topic?.name ? `<b>${infoData?.topic?.name}</b> dərsi` : ''} üzrə irəliləmə göstəriciləri.`}
                    filter={true}
                >
